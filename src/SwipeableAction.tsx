...
export const SwipeableAction = ({
   ...
   */
  const actionX = useTransform<number, number>(
    // @ts-expect-error: Wrong typings in framer-motion.
    [actionOffset, actionOverlayOffset],
    ([x, y]: [number, number]) => {
      if (position === "left") {
  ...
  // Animate to the middle after passing threshold if outer-most item
  useEffect(() => {
    ...
    return listItemXOffset.onChange((newValue) => {
      ...
      switch (position) {
        case "left":
          if (
            newValue > activationThreshold &&
            lastValue <= activationThreshold
          ) {
